...
args = {
'seq_root' :'../framework-master-thesis/data/newdata',
    ...
    for seq_idx, seq_name in enumerate(seq_list):
        ...
        for i,raw in enumerate(raws):
            ...
            # get mid idx 
                ...
                mid_idx = int(raw_length - (raw_length-i)/2)
            # load test image
            input = cv2.imread(raw)
            input = cv2.resize(input, (320, 384), cv2.INTER_AREA)
            
            target = cv2.imread(raws[0])
            target = cv2.resize(target, (320, 384), cv2.INTER_AREA)

            middle = cv2.imread(raws[mid_idx])
            middle = cv2.resize(middle, (320, 384), cv2.INTER_AREA)

            target = numpy2tensor(target)[None,:].to(device)
            input = numpy2tensor(input)[None,:].to(device)
            middle = numpy2tensor(middle)[None,:].to(device)

            # forward path
            with torch.no_grad():
                _, flow_e_m = model(middle, input, iters=50, test_mode=True)
                _, flow_m_s = model(target, middle, iters=50, test_mode=True)

            # flow warp
            flow_pr = warp(flow_e_m, flow_m_s) + flow_m_s

            # warp
            warped = warp(input, flow_pr)
            warped_mid = warp(input, flow_e_m)
            warped_end = warp(middle, flow_m_s)

            # convert from tensor to numpy
            input_np = tensor2numpy(input)
            middle_np = tensor2numpy(middle)
            target_np = tensor2numpy(target)
            flow_np = tensor2numpy(flow_pr)
            warped_np = tensor2numpy(warped)
            warped_mid_np = tensor2numpy(warped_mid)
            warped_end_np = tensor2numpy(warped_end)
            ...
            if True:
                ...
                # concate results
                save = np.hstack((input_np*255, middle_np*255, target_np*255, warped_np*255, warped_mid_np*255, warped_end_np*255))
                check_dir = os.path.join('check/flow1', str(seq_name))
