...
if __name__ == '__main__':
    ...
    for seq_idx, seq_folder in enumerate(seq_list):
        seq_folder = os.path.join(args['seq_root'], seq_folder)
        ...
        for i,raw in enumerate(raws):
            ...
            # resize to H//8 and W//8
            up_width = 320
            up_height = 480
            up_points = (up_width, up_height)
            input = cv2.resize(input, up_points, interpolation= cv2.INTER_LINEAR)
            target = cv2.resize(target, up_points, interpolation= cv2.INTER_LINEAR)
            ...
            if i % 10 == 0 :
                ...
                check_dir = os.path.join('check/flow', str(seq_idx))
