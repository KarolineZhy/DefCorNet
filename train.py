...
def create_train_val_dataloader(opt, logger):
    ...
    test_loader = build_dataloader(
        val_set, opt['datasets'], 'test', num_gpu=opt['num_gpu'], dist=opt['dist'], sampler=None,
        seed=opt['manual_seed'])
    logger.info('Testing statistics:'
                f'\n\tNumber of test images: {len(val_set)}')
    ...
    try:
        while model.curr_epoch <= total_epochs:
            ...
            for train_data in train_loader:
                ...
                if model.curr_iter % opt['logger']['train_log_freq'] == 0:
                    model.train_plot(tb_logger)
                ...
                # if opt.get('test') is not None and (model.curr_iter % opt['test']['test_freq'] == 0):
